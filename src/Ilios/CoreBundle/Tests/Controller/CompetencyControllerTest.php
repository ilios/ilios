...
class CompetencyControllerTest extends AbstractControllerTest
{
    ...
    {
        $competency = $this->container->get('ilioscore.dataloader.competency')->getOne()['competency'];
        $this->createJsonRequest('GET', $this->getUrl('get_competencies', ['id' => $competency['id']]));
    ...
    public function testPostCompetency()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.competency')->create()['competency'])
        );
    ...
    {
        $invalidCompetency = array_shift($this->container->get('ilioscore.dataloader.competency')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_competencies'), $invalidCompetency);
        $response = $this->client->getResponse();
    ...
    {
        $competency = $this->container->get('ilioscore.dataloader.competency')->createWithId()['competency'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_competencies', ['id' => $competency['id']]),
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.competency')->getLastCreated()['competency'],
            json_decode($response->getContent(), true)['competency']
        );
    }
    ...
    public function testDeleteCompetency()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_competencies', ['id' => $competency['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_competencies', ['id' => $competency['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_competencies', ['id' => '-9999']));
        $response = $this->client->getResponse();
