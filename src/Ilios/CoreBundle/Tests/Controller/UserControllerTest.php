...
class UserControllerTest extends AbstractControllerTest
{
    ...
    {
        $user = $this->container->get('ilioscore.dataloader.user')->getOne()['user'];
        $this->createJsonRequest('GET', $this->getUrl('get_users', ['id' => $user['id']]));
    ...
    public function testPostUser()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.user')->create()['user'])
        );
    ...
    {
        $invalidUser = array_shift($this->container->get('ilioscore.dataloader.user')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_users'), $invalidUser);
        $response = $this->client->getResponse();
    ...
    {
        $user = $this->container->get('ilioscore.dataloader.user')->createWithId()['user'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_users', ['id' => $user['id']]),
            json_encode($user)
        );
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.user')->getLastCreated()['user'],
            json_decode($response->getContent(), true)['user']
        );
    }
    ...
    public function testDeleteUser()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_users', ['id' => $user['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_users', ['id' => $user['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_users', ['id' => '-9999']));
        $response = $this->client->getResponse();
