...
class SessionControllerTest extends AbstractControllerTest
{
    ...
    {
        $session = $this->container->get('ilioscore.dataloader.session')->getOne()['session'];
        $this->createJsonRequest('GET', $this->getUrl('get_sessions', ['id' => $session['id']]));
    ...
    public function testPostSession()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.session')->create()['session'])
        );
    ...
    {
        $invalidSession = array_shift($this->container->get('ilioscore.dataloader.session')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_sessions'), $invalidSession);
        $response = $this->client->getResponse();
    ...
    {
        $session = $this->container->get('ilioscore.dataloader.session')->createWithId()['session'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_sessions', ['id' => $session['id']]),
            json_encode($session)
        );
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.session')->getLastCreated()['session'],
            json_decode($response->getContent(), true)['session']
        );
    }
    ...
    public function testDeleteSession()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_sessions', ['id' => $session['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_sessions', ['id' => $session['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_sessions', ['id' => '-9999']));
        $response = $this->client->getResponse();
