...
class ProgramControllerTest extends AbstractControllerTest
{
    ...
    {
        $program = $this->container->get('ilioscore.dataloader.program')->getOne()['program'];
        $this->createJsonRequest('GET', $this->getUrl('get_programs', ['id' => $program['id']]));
    ...
    public function testPostProgram()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.program')->create()['program'])
        );
    ...
    {
        $invalidProgram = array_shift($this->container->get('ilioscore.dataloader.program')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_programs'), $invalidProgram);
        $response = $this->client->getResponse();
    ...
    {
        $program = $this->container->get('ilioscore.dataloader.program')->createWithId()['program'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_programs', ['id' => $program['id']]),
            json_encode($program)
        );
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.program')->getLastCreated()['program'],
            json_decode($response->getContent(), true)['program']
        );
    }
    ...
    public function testDeleteProgram()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_programs', ['id' => $program['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_programs', ['id' => $program['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_programs', ['id' => '-9999']));
        $response = $this->client->getResponse();
