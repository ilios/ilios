...
class AlertControllerTest extends AbstractControllerTest
{
    ...
    {
        $alert = $this->container->get('ilioscore.dataloader.alert')->getOne()['alert'];
        $this->createJsonRequest('GET', $this->getUrl('get_alerts', ['id' => $alert['id']]));
    ...
    public function testPostAlert()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.alert')->create()['alert'])
        );
    ...
    {
        $invalidAlert = array_shift($this->container->get('ilioscore.dataloader.alert')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_alerts'), $invalidAlert);
        $response = $this->client->getResponse();
    ...
    {
        $alert = $this->container->get('ilioscore.dataloader.alert')->createWithId()['alert'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_alerts', ['id' => $alert['id']]),
            json_encode($alert)
        );
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.alert')->getLastCreated()['alert'],
            json_decode($response->getContent(), true)['alert']
        );
    }
    ...
    public function testDeleteAlert()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_alerts', ['id' => $alert['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_alerts', ['id' => $alert['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_alerts', ['id' => '-9999']));
        $response = $this->client->getResponse();
