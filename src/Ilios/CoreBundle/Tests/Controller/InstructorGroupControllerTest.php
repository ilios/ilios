...
class InstructorGroupControllerTest extends AbstractControllerTest
{
    ...
    {
        $instructorGroup = $this->container->get('ilioscore.dataloader.instructorgroup')->getOne()['instructorGroup'];
        $this->createJsonRequest('GET', $this->getUrl('get_instructorgroups', ['id' => $instructorGroup['id']]));
    ...
    public function testPostInstructorGroup()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.instructorgroup')->create()['instructorGroup'])
        );
    ...
    {
        $invalidInstructorGroup = array_shift($this->container->get('ilioscore.dataloader.instructorgroup')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_instructorgroups'), $invalidInstructorGroup);
        $response = $this->client->getResponse();
    ...
    {
        $instructorGroup = $this->container->get('ilioscore.dataloader.instructorgroup')->createWithId()['instructorGroup'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_instructorgroups', ['id' => $instructorGroup['id']]),
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.instructorgroup')->getLastCreated()['instructorGroup'],
            json_decode($response->getContent(), true)['instructorGroup']
        );
    }
    ...
    public function testDeleteInstructorGroup()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_instructorgroups', ['id' => $instructorGroup['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_instructorgroups', ['id' => $instructorGroup['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_instructorgroups', ['id' => '-9999']));
        $response = $this->client->getResponse();
