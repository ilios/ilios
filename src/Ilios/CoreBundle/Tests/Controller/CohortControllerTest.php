...
class CohortControllerTest extends AbstractControllerTest
{
    ...
    {
        $cohort = $this->container->get('ilioscore.dataloader.cohort')->getOne()['cohort'];
        $this->createJsonRequest('GET', $this->getUrl('get_cohorts', ['id' => $cohort['id']]));
    ...
    public function testPostCohort()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.cohort')->create()['cohort'])
        );
    ...
    {
        $invalidCohort = array_shift($this->container->get('ilioscore.dataloader.cohort')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_cohorts'), $invalidCohort);
        $response = $this->client->getResponse();
    ...
    {
        $cohort = $this->container->get('ilioscore.dataloader.cohort')->createWithId()['cohort'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_cohorts', ['id' => $cohort['id']]),
            json_encode($cohort)
        );
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.cohort')->getLastCreated()['cohort'],
            json_decode($response->getContent(), true)['cohort']
        );
    }
    ...
    public function testDeleteCohort()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_cohorts', ['id' => $cohort['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_cohorts', ['id' => $cohort['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_cohorts', ['id' => '-9999']));
        $response = $this->client->getResponse();
