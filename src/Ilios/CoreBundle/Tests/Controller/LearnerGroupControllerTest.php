...
class LearnerGroupControllerTest extends AbstractControllerTest
{
    ...
    {
        $learnerGroup = $this->container->get('ilioscore.dataloader.learnergroup')->getOne()['learnerGroup'];
        $this->createJsonRequest('GET', $this->getUrl('get_learnergroups', ['id' => $learnerGroup['id']]));
    ...
    public function testPostLearnerGroup()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.learnergroup')->create()['learnerGroup'])
        );
    ...
    {
        $invalidLearnerGroup = array_shift($this->container->get('ilioscore.dataloader.learnergroup')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_learnergroups'), $invalidLearnerGroup);
        $response = $this->client->getResponse();
    ...
    {
        $learnerGroup = $this->container->get('ilioscore.dataloader.learnergroup')->createWithId()['learnerGroup'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_learnergroups', ['id' => $learnerGroup['id']]),
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.learnergroup')->getLastCreated()['learnerGroup'],
            json_decode($response->getContent(), true)['learnerGroup']
        );
    }
    ...
    public function testDeleteLearnerGroup()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_learnergroups', ['id' => $learnerGroup['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_learnergroups', ['id' => $learnerGroup['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_learnergroups', ['id' => '-9999']));
        $response = $this->client->getResponse();
