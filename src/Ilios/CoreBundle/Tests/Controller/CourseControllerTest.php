...
class CourseControllerTest extends AbstractControllerTest
{
    ...
    {
        $course = $this->container->get('ilioscore.dataloader.course')->getOne()['course'];
        $this->createJsonRequest('GET', $this->getUrl('get_courses', ['id' => $course['id']]));
    ...
    public function testPostCourse()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.course')->create()['course'])
        );
    ...
    {
        $invalidCourse = array_shift($this->container->get('ilioscore.dataloader.course')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_courses'), $invalidCourse);
        $response = $this->client->getResponse();
    ...
    {
        $course = $this->container->get('ilioscore.dataloader.course')->createWithId()['course'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_courses', ['id' => $course['id']]),
            json_encode($course)
        );
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.course')->getLastCreated()['course'],
            json_decode($response->getContent(), true)['course']
        );
    }
    ...
    public function testDeleteCourse()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_courses', ['id' => $course['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_courses', ['id' => $course['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_courses', ['id' => '-9999']));
        $response = $this->client->getResponse();
