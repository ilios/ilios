...
class ObjectiveControllerTest extends AbstractControllerTest
{
    ...
    {
        $objective = $this->container->get('ilioscore.dataloader.objective')->getOne()['objective'];
        $this->createJsonRequest('GET', $this->getUrl('get_objectives', ['id' => $objective['id']]));
    ...
    public function testPostObjective()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.objective')->create()['objective'])
        );
    ...
    {
        $invalidObjective = array_shift($this->container->get('ilioscore.dataloader.objective')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_objectives'), $invalidObjective);
        $response = $this->client->getResponse();
    ...
    {
        $objective = $this->container->get('ilioscore.dataloader.objective')->createWithId()['objective'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_objectives', ['id' => $objective['id']]),
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.objective')->getLastCreated()['objective'],
            json_decode($response->getContent(), true)['objective']
        );
    }
    ...
    public function testDeleteObjective()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_objectives', ['id' => $objective['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_objectives', ['id' => $objective['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_objectives', ['id' => '-9999']));
        $response = $this->client->getResponse();
