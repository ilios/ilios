...
class ProgramYearControllerTest extends AbstractControllerTest
{
    ...
    {
        $programYear = $this->container->get('ilioscore.dataloader.programyear')->getOne()['programYear'];
        $this->createJsonRequest('GET', $this->getUrl('get_programyears', ['id' => $programYear['id']]));
    ...
    public function testPostProgramYear()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.programyear')->create()['programYear'])
        );
    ...
    {
        $invalidProgramYear = array_shift($this->container->get('ilioscore.dataloader.programyear')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_programyears'), $invalidProgramYear);
        $response = $this->client->getResponse();
    ...
    {
        $programYear = $this->container->get('ilioscore.dataloader.programyear')->createWithId()['programYear'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_programyears', ['id' => $programYear['id']]),
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.programyear')->getLastCreated()['programYear'],
            json_decode($response->getContent(), true)['programYear']
        );
    }
    ...
    public function testDeleteProgramYear()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_programyears', ['id' => $programYear['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_programyears', ['id' => $programYear['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_programyears', ['id' => '-9999']));
        $response = $this->client->getResponse();
