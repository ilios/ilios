...
class SessionTypeControllerTest extends AbstractControllerTest
{
    ...
    {
        $sessionType = $this->container->get('ilioscore.dataloader.sessiontype')->getOne()['sessionType'];
        $this->createJsonRequest('GET', $this->getUrl('get_sessiontypes', ['id' => $sessionType['id']]));
    ...
    public function testPostSessionType()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.sessiontype')->create()['sessionType'])
        );
    ...
    {
        $invalidSessionType = array_shift($this->container->get('ilioscore.dataloader.sessiontype')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_sessiontypes'), $invalidSessionType);
        $response = $this->client->getResponse();
    ...
    {
        $sessionType = $this->container->get('ilioscore.dataloader.sessiontype')->createWithId()['sessionType'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_sessiontypes', ['id' => $sessionType['id']]),
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.sessiontype')->getLastCreated()['sessionType'],
            json_decode($response->getContent(), true)['sessionType']
        );
    }
    ...
    public function testDeleteSessionType()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_sessiontypes', ['id' => $sessionType['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_sessiontypes', ['id' => $sessionType['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_sessiontypes', ['id' => '-9999']));
        $response = $this->client->getResponse();
