...
class SchoolControllerTest extends AbstractControllerTest
{
    ...
    {
        $school = $this->container->get('ilioscore.dataloader.school')->getOne()['school'];
        $this->createJsonRequest('GET', $this->getUrl('get_schools', ['id' => $school['id']]));
    ...
    public function testPostSchool()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.school')->create()['school'])
        );
    ...
    {
        $invalidSchool = array_shift($this->container->get('ilioscore.dataloader.school')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_schools'), $invalidSchool);
        $response = $this->client->getResponse();
    ...
    {
        $school = $this->container->get('ilioscore.dataloader.school')->createWithId()['school'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_schools', ['id' => $school['id']]),
            json_encode($school)
        );
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.school')->getLastCreated()['school'],
            json_decode($response->getContent(), true)['school']
        );
    }
    ...
    public function testDeleteSchool()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_schools', ['id' => $school['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_schools', ['id' => $school['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_schools', ['id' => '-9999']));
        $response = $this->client->getResponse();
