...
class IlmSessionFacetControllerTest extends AbstractControllerTest
{
    ...
    {
        $ilmSessionFacet = $this->container->get('ilioscore.dataloader.ilmsessionfacet')->getOne()['ilmSessionFacet'];
        $this->createJsonRequest('GET', $this->getUrl('get_ilmsessionfacets', ['id' => $ilmSessionFacet['id']]));
    ...
    public function testPostIlmSessionFacet()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.ilmsessionfacet')->create()['ilmSessionFacet'])
        );
    ...
    {
        $invalidIlmSessionFacet = array_shift($this->container->get('ilioscore.dataloader.ilmsessionfacet')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_ilmsessionfacets'), $invalidIlmSessionFacet);
        $response = $this->client->getResponse();
    ...
    {
        $ilmSessionFacet = $this->container->get('ilioscore.dataloader.ilmsessionfacet')->createWithId()['ilmSessionFacet'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_ilmsessionfacets', ['id' => $ilmSessionFacet['id']]),
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.ilmsessionfacet')->getLastCreated()['ilmSessionFacet'],
            json_decode($response->getContent(), true)['ilmSessionFacet']
        );
    }
    ...
    public function testDeleteIlmSessionFacet()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_ilmsessionfacets', ['id' => $ilmSessionFacet['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_ilmsessionfacets', ['id' => $ilmSessionFacet['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_ilmsessionfacets', ['id' => '-9999']));
        $response = $this->client->getResponse();
