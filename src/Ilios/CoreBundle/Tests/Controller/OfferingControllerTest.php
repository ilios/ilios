...
class OfferingControllerTest extends AbstractControllerTest
{
    ...
    {
        $offering = $this->container->get('ilioscore.dataloader.offering')->getOne()['offering'];
        $this->createJsonRequest('GET', $this->getUrl('get_offerings', ['id' => $offering['id']]));
    ...
    public function testPostOffering()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.offering')->create()['offering'])
        );
    ...
    {
        $invalidOffering = array_shift($this->container->get('ilioscore.dataloader.offering')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_offerings'), $invalidOffering);
        $response = $this->client->getResponse();
    ...
    {
        $offering = $this->container->get('ilioscore.dataloader.offering')->createWithId()['offering'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_offerings', ['id' => $offering['id']]),
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.offering')->getLastCreated()['offering'],
            json_decode($response->getContent(), true)['offering']
        );
    }
    ...
    public function testDeleteOffering()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_offerings', ['id' => $offering['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_offerings', ['id' => $offering['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_offerings', ['id' => '-9999']));
        $response = $this->client->getResponse();
