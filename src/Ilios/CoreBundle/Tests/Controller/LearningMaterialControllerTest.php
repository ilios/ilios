...
class LearningMaterialControllerTest extends AbstractControllerTest
{
    ...
    {
        $learningMaterial = $this->container->get('ilioscore.dataloader.learningmaterial')->getOne()['learningMaterial'];
        $this->createJsonRequest('GET', $this->getUrl('get_learningmaterials', ['id' => $learningMaterial['id']]));
    ...
    public function testPostLearningMaterial()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.learningmaterial')->create()['learningMaterial'])
        );
    ...
    {
        $invalidLearningMaterial = array_shift($this->container->get('ilioscore.dataloader.learningmaterial')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_learningmaterials'), $invalidLearningMaterial);
        $response = $this->client->getResponse();
    ...
    {
        $learningMaterial = $this->container->get('ilioscore.dataloader.learningmaterial')->createWithId()['learningMaterial'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_learningmaterials', ['id' => $learningMaterial['id']]),
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.learningmaterial')->getLastCreated()['learningMaterial'],
            json_decode($response->getContent(), true)['learningMaterial']
        );
    }
    ...
    public function testDeleteLearningMaterial()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_learningmaterials', ['id' => $learningMaterial['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_learningmaterials', ['id' => $learningMaterial['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_learningmaterials', ['id' => '-9999']));
        $response = $this->client->getResponse();
