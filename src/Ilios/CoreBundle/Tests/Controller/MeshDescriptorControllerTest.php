...
class MeshDescriptorControllerTest extends AbstractControllerTest
{
    ...
    {
        $meshDescriptor = $this->container->get('ilioscore.dataloader.meshdescriptor')->getOne()['meshDescriptor'];
        $this->createJsonRequest('GET', $this->getUrl('get_meshdescriptors', ['id' => $meshDescriptor['id']]));
    ...
    public function testPostMeshDescriptor()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.meshdescriptor')->create()['meshDescriptor'])
        );
    ...
    {
        $invalidMeshDescriptor = array_shift($this->container->get('ilioscore.dataloader.meshdescriptor')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_meshdescriptors'), $invalidMeshDescriptor);
        $response = $this->client->getResponse();
    ...
    {
        $meshDescriptor = $this->container->get('ilioscore.dataloader.meshdescriptor')->createWithId()['meshDescriptor'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_meshdescriptors', ['id' => $meshDescriptor['id']]),
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.meshdescriptor')->getLastCreated()['meshDescriptor'],
            json_decode($response->getContent(), true)['meshDescriptor']
        );
    }
    ...
    public function testDeleteMeshDescriptor()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_meshdescriptors', ['id' => $meshDescriptor['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_meshdescriptors', ['id' => $meshDescriptor['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_meshdescriptors', ['id' => '-9999']));
        $response = $this->client->getResponse();
