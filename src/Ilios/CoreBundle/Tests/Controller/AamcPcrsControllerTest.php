...
class AamcPcrsControllerTest extends AbstractControllerTest
{
    ...
    {
        $aamcPcrs = $this->container->get('ilioscore.dataloader.aamcpcrs')->getOne()['aamcPcrs'];
        $this->createJsonRequest('GET', $this->getUrl('get_aamcpcrs', ['id' => $aamcPcrs['id']]));
    ...
    public function testPostAamcPcrs()
    {
        $this->createJsonRequest(
            ...
            json_encode($this->container->get('ilioscore.dataloader.aamcpcrs')->create()['aamcPcrs'])
        );
    ...
    {
        $invalidAamcPcrs = array_shift($this->container->get('ilioscore.dataloader.aamcpcrs')->invalid());
        $this->createJsonRequest('POST', $this->getUrl('post_aamcpcrs'), $invalidAamcPcrs);
        $response = $this->client->getResponse();
    ...
    {
        $aamcPcrs = $this->container->get('ilioscore.dataloader.aamcpcrs')->createWithId()['aamcPcrs'];

        $this->createJsonRequest(
            'PUT',
            $this->getUrl('put_aamcpcrs', ['id' => $aamcPcrs['id']]),
        ...
        $this->assertEquals(
            $this->container->get('ilioscore.dataloader.aamcpcrs')->getLastCreated()['aamcPcrs'],
            json_decode($response->getContent(), true)['aamcPcrs']
        );
    }
    ...
    public function testDeleteAamcPcrs()
    {
        ...
        $response = $this->client->getResponse();
        $this->assertJsonResponse($response, Codes::HTTP_CREATED);
        $this->client->request('DELETE', $this->getUrl('delete_aamcpcrs', ['id' => $aamcPcrs['id']]));
        $response = $this->client->getResponse();
        $this->assertEquals(Codes::HTTP_NO_CONTENT, $response->getStatusCode());
        $this->client->request('GET', $this->getUrl('get_aamcpcrs', ['id' => $aamcPcrs['id']]));
        $response = $this->client->getResponse();
    ...
    {
        $this->createJsonRequest('GET', $this->getUrl('get_aamcpcrs', ['id' => '-9999']));
        $response = $this->client->getResponse();
