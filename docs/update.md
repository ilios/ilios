...
To help with migration, we have added a `TRANSITIONAL_ILIOS_SECRET` [environment variable](./env_vars_and_config.md)
...
To ensure long-term viability of this key, we recommend choosing a random value at least 64 characters long.
The Ilios team recommends using a value generated by `php -r 'echo bin2hex(random_bytes(32));'`
...
The `ILIOS_SEARCH_HOSTS` has been renamed to `ILIOS_SEARCH_HOST` and will no longer accept a semi-colon separated list of hosts. Replace this is your configuration with a single host if you have search and indexing enabled.
