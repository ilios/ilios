...
use Ilios\AuthenticationBundle\RelationshipVoter\InstructorGroup as Voter;
...
class InstructorGroupTest extends AbstractBase
{
    ...
    {
        $this->checkRootAccess(InstructorGroup::class, InstructorGroupDTO::class);
    }

    public function testCanViewDTO()
    {
        $token = $this->createMockTokenWithNonRootSessionUser();
        $dto = m::mock(InstructorGroupDTO::class);
        $dto->id = 1;
        $dto->school = 1;
        $this->permissionChecker->shouldReceive('canReadInstructorGroup')->andReturn(true);
    ...
    public function testCanNotViewDTO()
    {
        ...
        $dto->id = 1;
        $dto->school = 1;
        $this->permissionChecker->shouldReceive('canReadInstructorGroup')->andReturn(false);
        $response = $this->voter->vote($token, $dto, [AbstractVoter::VIEW]);
    ...
    {
        $token = $this->createMockTokenWithNonRootSessionUser();
        $entity = m::mock(InstructorGroup::class);
