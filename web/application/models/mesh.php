...
class Mesh extends Abstract_Ilios_Model
{
    ...
    public function saveMeSHSearchSelection ($pairArray)
    {
        ...
        foreach ($pairArray as $pair) {
            if (! is_null($pair['uid'])) {
                ...
                $DB->insert('mesh_user_selection', $newRow);

                $affectedCount += $DB->affected_rows();
                $count++;
            }
        }
    ...
    public function searchMeSHUniverseForIlios ($searchString)
    {
        ...
        $DB->like('search_phrase', $searchString);
        $queryResults = $DB->get('mesh_user_selection');
        ...
        $queryResults = $DB->query($meshSearchProcedureInvocation);
    ...
    public function getMeSHObjectForDescriptor ($descriptorUniqueId, $treeNumber = null)
    {
        ...
        $DB->where('mesh_descriptor_uid', $descriptorUniqueId);
        $queryResults = $DB->get('mesh_descriptor');
        ...
        $conceptIds = array();
        $DB->where('mesh_descriptor_uid', $descriptorUniqueId);
        $queryResults = $DB->get('mesh_descriptor_x_concept');
        foreach ($queryResults->result_array() as $row) {
            array_push($conceptIds, $row['mesh_concept_uid']);
        }
        $scopeNotes = array();
        foreach ($conceptIds as $conceptId) {
            $DB->where('mesh_concept_uid', $conceptId);
            $queryResults = $DB->get('mesh_concept');
            foreach ($queryResults->result_array() as $row) {
        ...
        if (! is_null($treeNumber)) {
            ...
            $queryResults = $DB->query($meshTreeDecomposeInvocation);
            foreach ($queryResults->result_array() as $row) {
    ...
    protected function getMeSHObjectForTreeNumber ($treeNumber, $descriptorUniqueId = null,
        ...
        if (is_null($descriptorUniqueId)) {
            $DB->where('tree_number', $treeNumber);
            $queryResults = $DB->get('mesh_tree_x_descriptor');
            foreach ($queryResults->result_array() as $row) {
