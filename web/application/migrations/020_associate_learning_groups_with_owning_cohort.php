...
/**
 * Breaks up the `cohort_master_group` JOIN table and moves the `cohort_id` directly into the `group` table.
 */
class Migration_Associate_learning_groups_with_owning_cohort extends CI_Migration
{
    ...
    {
        $this->db->trans_start();
        // add the `cohort_id` column to the `group` table, we'll add the foreign key constraint after the next step or
        // it will error
        $sql = <<<EOL
ALTER TABLE `group`
