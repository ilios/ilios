...
class Migration_populate_aamc_vocabulary_tables extends CI_Migration
{
    ...
    public function up ()
    {
        ...
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0101','Provide patient care that is compassionate, appropriate, and effective for the treatment of health problems and the promotion of health')");
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0102','Perform all medical, diagnostic, and surgical procedures considered essential for the area of practice')");
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0103',' Gather essential and accurate information about patients and their condition through history-taking, physical examination, and the use of laboratory data, imaging and other tests')");
        ...
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0105','Counsel and educate patients and their families to empower them to participate in their care, showing consideration for their perspective throughout treatment')");
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0106','Make informed decisions about diagnostic and therapeutic interventions based on patient information and preferences, up-to-date scientific evidence, and clinical judgment')");
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0107','Develop and carry out patient management plans')");
        ...
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0303','Identify and perform learning activities that address one’s gaps in knowledge, skills or attitudes')");
        ...
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0309','Use information technology to obtain and utilize information about individual patients, populations of patients being served or communities from which patients are drawn to improve care')");
        ...
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0402','Communicate effectively with colleagues within one’s profession or specialty, other health professionals, and health related agencies')");
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0403','Work effectively with others as a member or leader of a health care team or other professional group')");
        ...
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0406','Demonstrate sensitivity, honesty, and compassion in difficult conversations about issues such as death, end-of-life issues, adverse events, bad news, disclosure of errors, and other sensitive topics')");
        ...
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0601','Work effectively in various health care delivery settings and systems relevant to their clinical specialty')");
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0602','Coordinate patient care within the health care system relevant to their clinical specialty')");
        ...
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0605','Work in interprofessional teams to enhance patient safety and improve patient care quality')");
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0606','Participate in identifying system errors and implementing potential systems solutions')");
        ...
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0702','Utilize and enhance one’s own expertise by understanding and engaging the unique and diverse knowledge, skills, and abilities of other professionals to enhance team performance and maximize the quality of patient care')");
        ...
        $this->db->query("INSERT INTO `aamc_pcrs` (`pcrs_id`, `description`) VALUES ('aamc-pcrs-comp-c0804','Practice flexibility and maturity in adjusting to change with the capacity to alter behavior')");
...
DELETE FROM `aamc_pcrs` WHERE `pcrs_id` IN ('aamc-pcrs-comp-c0101', 'aamc-pcrs-comp-c0102', 'aamc-pcrs-comp-c0103',
    'aamc-pcrs-comp-c0104', 'aamc-pcrs-comp-c0105', 'aamc-pcrs-comp-c0106', 'aamc-pcrs-comp-c0107', 'aamc-pcrs-comp-c0108',
    'aamc-pcrs-comp-c0109', 'aamc-pcrs-comp-c0201', 'aamc-pcrs-comp-c0202', 'aamc-pcrs-comp-c0203', 'aamc-pcrs-comp-c0204',
    'aamc-pcrs-comp-c0205', 'aamc-pcrs-comp-c0206', 'aamc-pcrs-comp-c0301', 'aamc-pcrs-comp-c0302', 'aamc-pcrs-comp-c0303',
    'aamc-pcrs-comp-c0304', 'aamc-pcrs-comp-c0305', 'aamc-pcrs-comp-c0306', 'aamc-pcrs-comp-c0307', 'aamc-pcrs-comp-c0308',
    'aamc-pcrs-comp-c0309', 'aamc-pcrs-comp-c0310', 'aamc-pcrs-comp-c0401', 'aamc-pcrs-comp-c0402', 'aamc-pcrs-comp-c0403',
    'aamc-pcrs-comp-c0404', 'aamc-pcrs-comp-c0405', 'aamc-pcrs-comp-c0406', 'aamc-pcrs-comp-c0501', 'aamc-pcrs-comp-c0502',
    'aamc-pcrs-comp-c0503', 'aamc-pcrs-comp-c0504', 'aamc-pcrs-comp-c0505', 'aamc-pcrs-comp-c0506', 'aamc-pcrs-comp-c0601',
    'aamc-pcrs-comp-c0602', 'aamc-pcrs-comp-c0603', 'aamc-pcrs-comp-c0604', 'aamc-pcrs-comp-c0605', 'aamc-pcrs-comp-c0606',
    'aamc-pcrs-comp-c0701', 'aamc-pcrs-comp-c0702', 'aamc-pcrs-comp-c0703', 'aamc-pcrs-comp-c0704', 'aamc-pcrs-comp-c0801',
    'aamc-pcrs-comp-c0802', 'aamc-pcrs-comp-c0803', 'aamc-pcrs-comp-c0804', 'aamc-pcrs-comp-c0805', 'aamc-pcrs-comp-c0806',
    'aamc-pcrs-comp-c0807', 'aamc-pcrs-comp-c0808')
EOL;
        $this->db->query($sql);
        $this->db->trans_complete();
    }
