 ...
 */
class Ilios_UserSync_Process_StudentProcess extends Ilios_UserSync_Process
{
    /**
     ...
     * @var string
     */
    const SIGNATURE = 'Ilios_UserSync_Process_StudentProcess';
            ...
            foreach ($externalStudents as $externalStudent) {
                ...
                if ($this->_hasMatchingUsersInIlios($externalStudent)) {
                    ...
                    try {
                        ...
                        $logger->log($msg, $processId, 1);
                    } catch (Ilios_UserSync_Exception $e2) {
                        // log the incident
                        $logger->log($e2->getMessage(), $processId, 1, Ilios_Logger::LOG_LEVEL_ERROR);
                    }
                }
    ...
    protected function _resetExaminedFlagOnStudents ()
    {
        ...
        if ($failedTransaction) {
            throw new Ilios_UserSync_Exception("Failed to reset 'examined' flag on existing users.");
        }
    }
