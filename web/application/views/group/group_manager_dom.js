...
ilios.gm.handleGroupDivCollapse = function (containerNumber, summaryTextDiv) {
    ...
    summaryTextDiv.innerHTML = groupModel.getTitle() +
        ...
        i18nStr + ': <span id="' +
        ilios.gm.generateIdForEnrollmentSummary(containerNumber) +
...
ilios.gm.generateIdForEnrollmentSummary = function (containerNumber) {
    return ilios.gm.generateIdForEnrollment(containerNumber) + '_summary';
};
...
/**
 * Turn the div created in cohort_content_container_include.php into a dialog
 * @method createEmptyGroupDialog
 * @param {Function} a method to handle the save button action
 *
 */
ilios.gm.emptyGroupDialog = null;
ilios.gm.createEmptyGroupDialog = function(handleSave){
    ...
    var dialog = new YAHOO.widget.Dialog('empty_group_add_dialog', {
        width: panelWidth,
        visible: false,
    ...
    dialog.setHeader(ilios_i18nVendor.getI18NString("groups.empty_group_dialog.title"));
    dialog.render();
    dialog.center();
    dialog.show();
    ilios.gm.emptyGroupDialog = dialog;
};
