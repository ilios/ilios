...
class Cron_Tasks extends Ilios_Base_Controller
{
     ...
     * Default action.
     *
     * This method triggers all enabled, non-daily tasks to run.
     *
     * It prints out the task status information or exceptions generated by the subroutines that are called from within
     * this method.
     */
    ...
    /**
     * This method kicks off configured tasks that are meant  to run once a day.
     *
     * It prints out the task status information or exceptions generated by the subroutines that are called from within
     * this method.
     */
     ...
     * Sync Users Action
     *
     * This method runs only the user sync task, and only if it is enabled
     *
     * It prints out the task status information or exceptions generated by the subroutines that are called from within
     * this method.
     */
    ...
    /**
     * This method kicks off the change alert notification process.
     *
    ...
    /**
     * This method kicks off the user synchronization processes.
     *
     * @param array $config The process configuration.
    ...
    protected function _triggerUserSyncProcesses (array $config)
    {
        ...
        $studentSyncProcess = new Ilios_UserSync_Process_StudentProcess(
            $userSource,
            $this->user,
            $this->school,
            $this->userSyncException
        );
        $processId = time();
        ...
        $processId = time();
        $nonStudentSyncProcess = new Ilios_UserSync_Process_NonStudentProcess(
            $userSource,
            $this->user,
            $this->userSyncException
        );
        $nonStudentSyncProcess->run($processId, $logger);
        unset($nonStudentSyncProcess);
    ...
    /**
     * This method exports enrollment information to file for ingestion into CLEAE through an external process.
     *
    ...
    /**
     * This method sends out reminder alerts for upcoming session offerings.
     *
     * @deprecated
     * @todo Reimplement according to new rules, see ticket #1009.
     */
    protected function _processOfferingReminders ()
    {
        ...
        foreach ($offerings as $offering) {
            $this->_handleOfferingReminder($offering);
        }
    }


    /**
     * This method sends out a reminder email to all instructors and learners participating in a given session offering.
     *
     ...
     * @todo Reimplement according to new rules, see ticket #1009.
     */
    protected function _handleOfferingReminder($offering)
    {
    ...
    /**
     * This method populates the given email template with the given reminder information and mails it out to a given
     * list of recipients.
     *
     ...
     *
     * @deprecated
     * @todo Sending out email in real-time in a loop does not scale at all - reimplement this from scratch.
     */
    protected function _mergeTemplateAndMailGroup($recipients, $subject, $template, $tokens, $values, $sender)
    {
        $count = 0;
