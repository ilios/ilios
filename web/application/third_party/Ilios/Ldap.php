...
class Ilios_Ldap
{
    ...
    public function connect ()
    {
        ...
        $errorMsg = "Failed to connect to {$host}:{$port}: " . ldap_err2str($errorNumber);
        $this->disconnect();
    ...
    public function bind ()
    {
        ...
        $errorMsg = "LDAP bind failed: " . ldap_err2str($errorNumber);
        $this->disconnect();
        throw new Ilios_Ldap_Exception($errorMsg, $errorNumber);
    }
                    ...
                    $attrOnly = false, $limit = 0, $timeout = 0)
    {
        ...
        if (false === $result) {
            $errorNumber = @ldap_errno($this->_ldap);
            throw new Ilios_Ldap_Exception('LDAP search failed: ' . ldap_err2str($errorNumber), $errorNumber);
        }
        return $result;
    }
